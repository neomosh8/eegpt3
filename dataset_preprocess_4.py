...
def _process_single_group(args):
    ...
    for region in REGIONS:
        ...
        # Read the original tokens from the file, prepend the TRIAL_TOKEN,
        # then split and convert to integers.
        with open(local_path, 'r', encoding='utf-8') as f:
            tokens_str = f.read().strip()
        # Prepend the special trial token (as a string) followed by a space.
        tokens_str = f"{TRIAL_TOKEN} " + tokens_str
        # Split the tokens (assuming they are separated by whitespace)
        token_list = tokens_str.split()
        # Convert tokens to integers.
        token_ints = [int(token) for token in token_list]
        tokens_tensor = torch.tensor(token_ints, dtype=torch.long)
